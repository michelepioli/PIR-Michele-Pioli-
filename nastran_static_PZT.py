...
class NastranStaticPZT(ExternalCode):
    template_file = 'nastran_static_PZT_template.inp'
    ...
    def __init__(self, node_id, node_id_all, n_stress, tn, tpn, mn, case_name):
        ...
        self.add_param('V', val=np.zeros(self.tpn))
    ...
    def create_input_file(self, params):
        ...
        #Compute equivalent alpha from d31 and tp
        alpha = d31/tp
        ...
        #Assign each PZT equivalent alpha value to its corresponding ID in the input data dictionary
        for i in range(len(tp)):
            input_data['alpha'+str(i+1)] = print_float_8(alpha[i])
